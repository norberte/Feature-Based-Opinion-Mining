...
def main(SWN_FILENAME):
	...
	for filename in glob("list of products/*.txt"):
		...
		while len(reviews) > 0:
			...
			sentences = review.split(".")  # splitting the review on the basis of fullstop.
			...
			for i in sentences:
				...
				final_sentences.extend(
					hf.check_for_but(i))  # split the sentances having but in them to 2 diff sentances
			for t in final_sentences:  # scan the final set of sentances to check for features
				...
				for i in feature_name:
					if i.strip(".,?/-").lower() not in review_category:
		...
		if len(list(review_category.keys())) == 0:
